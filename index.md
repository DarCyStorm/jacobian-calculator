...
### Implementation
you can see the main code here:
```python
from tkinter import *
...
root.configure(background="#BF40BF")
...
def Jacobi_Starts():
    try:
        ...
        def Single_Equation_Simplification(equation_number):
            ...
            new_equation = new_equation[equal_index-1] + "=(" + new_equation[(equal_index+1):] + ")/(" + \
                new_equation[:(equal_index-1)].replace("*", "") + ")"
            equation_number -= 1
            equation_list[equation_number] = new_equation

        def Get_Tripple_Input():
...
input_number_of_equation = Text(root, width=40, height=5, bg="white", fg="black", font=(
    "Helvetica", 16))
...
confirm_button = Button(root, text="Click To Calculate",
                        height=2, command=Jacobi_Starts, font=("Helvetica", 16))
...
answer = Label(root, height=5, text="        ")
answer.pack(pady=5)

h = Scrollbar(answer, orient='horizontal')
h.pack(side=BOTTOM, fill=X)
t = Text(answer, height=5, width=30, wrap=NONE,
...
tp = Text(final_ans, height=5, width=30, wrap=NONE,
